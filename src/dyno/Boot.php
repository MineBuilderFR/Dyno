...
namespace dyno {
    ...
    use dyno\utils\{
        MainLogger, Terminal, Utils
    };
    use dyno\works\{
        CompatibleClassLoader, ThreadManager
    };

    const NAME = "Dyno";
    const VERSION = "1.0";
    const API_VERSION = "1.0.0";
    const CODENAME = "SugarLaunch";

    if (\Phar::running(true) !== "") {
        @define('dyno\PATH', \Phar::running(true) . "/");
    } else {
        @define('dyno\PATH', \getcwd() . DIRECTORY_SEPARATOR);
    }
    ...
    if (!class_exists("ClassLoader", false)) {
        require_once(\dyno\PATH . "src/spl/ClassLoader.php");
        require_once(\dyno\PATH . "src/spl/BaseClassLoader.php");
        require_once(\dyno\PATH . "src/dyno/works/CompatibleClassLoader.php");
    }

    $autoloader = new CompatibleClassLoader();
    $autoloader->addPath(\dyno\PATH . "src");
    $autoloader->addPath(\dyno\PATH . "src" . DIRECTORY_SEPARATOR . "spl");
    ...
    define('dyno\START_TIME', microtime(true));

    $opts = getopt("", ["data:", "plugins:"]);

    define('dyno\DATA', isset($opts["data"]) ? $opts["data"] . DIRECTORY_SEPARATOR : \getcwd() . DIRECTORY_SEPARATOR);
    define('dyno\PLUGIN_PATH', isset($opts["plugins"]) ? $opts["plugins"] . DIRECTORY_SEPARATOR : \getcwd() . DIRECTORY_SEPARATOR . "plugins" . DIRECTORY_SEPARATOR);

    Terminal::init();

    define('dyno\ANSI', Terminal::hasFormattingCodes());

    if (!file_exists(\dyno\DATA)) {
        mkdir(\dyno\DATA, 0777, true);
    }

    //Logger has a dependency on timezone, so we'll set it to UTC until we can get the actual timezone.
    date_default_timezone_set("UTC");

    $logger = new MainLogger(\dyno\DATA . "server.log", true);
    ...
    {
        return rtrim(str_replace(["\\", ".php", "phar://", rtrim(str_replace(["\\", "phar://"], ["/", ""], \dyno\PATH), "/"), rtrim(str_replace(["\\", "phar://"], ["/", ""], \dyno\PLUGIN_PATH), "/")], ["/", "", "", "", ""], $path), "/");
    }

    $errors = 0;

    if (php_sapi_name() !== "cli") {
        $logger->critical("You must run " . \dyno\NAME . " using the CLI.");
        ++$errors;
    }
    ...
    if (file_exists(\dyno\PATH . ".git/refs/heads/master")) { //Found Git information!
        define('dyno\GIT_COMMIT', strtolower(trim(file_get_contents(\dyno\PATH . ".git/refs/heads/master"))));
    } else { //Unknown :(
        define('dyno\GIT_COMMIT', str_repeat("00", 20));
    }
    ...
    ThreadManager::init();
    new Server($autoloader, $logger, \dyno\PATH, \dyno\DATA, \dyno\PLUGIN_PATH);
    ...
    echo "Dyno has stopped" . Terminal::$FORMAT_RESET . "\n";
